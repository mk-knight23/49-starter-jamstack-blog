...
import { ArrowLeft, Clock, Tag } from 'lucide-vue-next'

const route = useRoute()
const store = usePostsStore()

const tag = computed(() => route.params.tag as string)
const posts = computed(() => store.posts.filter(p => p.tags?.includes(tag.value)))
  ...
  <div class="max-w-4xl mx-auto px-8 py-16 space-y-12">
    ...
    <header class="space-y-4">
      <div class="flex items-center gap-3">
        <Tag class="text-cinema-gold" :size="24" />
        <h1 class="text-5xl font-display font-black">
          Posts tagged with <span class="text-cinema-gold">#{{ tag }}</span>
        </h1>
      </div>
        ...
        <router-link :to="`/post/${post.id}`" class="block space-y-4">
          ...
          <div v-if="post.tags" class="flex flex-wrap gap-2">
            <span
              ...
              :key="t"
              class="px-3 py-1 rounded-full font-mono text-[10px] uppercase tracking-wider border border-cinema-border hover:border-cinema-gold transition-colors"
              :class="t === tag ? 'bg-cinema-gold text-cinema-bg' : 'text-cinema-muted'"
            >
              #{{ t }}
            </span>
          </div>
    ...
    <div v-if="posts.length === 0" class="text-center py-20">
      <p class="text-xl text-cinema-muted">No posts found with this tag.</p>
